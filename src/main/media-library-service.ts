...
async function downloadThumbnail(url: string, destPath: string): Promise<void> {
  // Thumbnail URLs come from the remote media API. guardedFetch applies the
  // transport policy to the initial URL and to every redirect hop, and throws
  // TransportRefusedError rather than fetching when it refuses.
  const res = await guardedFetch(url, undefined, { timeoutMs: ASSET_TIMEOUT_MS })
