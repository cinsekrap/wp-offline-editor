...
export function PostListSidebar({
  ...
  onSelectPost,
  onNewPost
}: PostListSidebarProps): JSX.Element {
  ...
  const filteredAndSorted = useMemo(() => {
    let result = activeFilters.size === 0 ? posts : posts.filter((p) => activeFilters.has(p.status))
    ...
    return result
  }, [posts, activeFilters, activeAuthorFilters, syncFilter, sortBy])
        ...
        <div className="flex items-center gap-0.5">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="ghost" size="icon" className="h-7 w-7 relative" title="Filter by status">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="ghost" size="icon" className="h-7 w-7" title="Sort posts">
          ...
          <Button variant="ghost" size="icon" className="h-7 w-7" onClick={onNewPost} title="New post">
              ...
              >
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  {!post.synced && !post.conflict && (
                    <CloudUpload className="h-3 w-3 text-blue-500 shrink-0" title="Not synced" />
                  )}
                  {post.synced && !post.conflict && (
